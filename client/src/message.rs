...
pub fn stringify_message_object(message: Message) -> String {
    return [message.destination, "|".to_owned(), message.source, "|".to_owned(), message.message, "|".to_owned(), message.message_len.to_string(), "|".to_owned(), message.timestamp.to_string(), "|".to_owned(), message.sequence_number.to_string()].concat();
}
...
pub fn receive_string_to_message_object(received_message: String) -> Message{
    ...
    let message = Message{
        ...
        timestamp: timestamp,
        sequence_number: res[5].to_owned().parse::<usize>().unwrap(),
    };
    return message;
}

pub fn get_message_to_send(destination: String, source: String, message:String, key_encryption:Vec<u8>, key_signature:String, sequence_number:usize) -> String {
